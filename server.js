...
const dbReady = db.initSqlite().then(() => seedDemoData()).catch((err) => {
...
async function seedDemoData() {
  ...
  const order = await db.getOne(db.usePostgres ? 'SELECT id FROM orders WHERE buyer_email = $1 LIMIT 1' : 'SELECT id FROM orders WHERE buyer_email = ? LIMIT 1', [demoEmail]);
  if (!order) {
    await db.run(db.usePostgres
      ...
      [demoEmail, 'Pacote Neural X', 'Disponível', 29.90, '/assets/neural-collection.svg', '/produto-neural-x.html']);
  }
}
...
async function createUser(email, hash, role='client', name='Cliente Neural X') {
  if (db.usePostgres) {
    return db.run('INSERT INTO users (email, password_hash, role, name) VALUES ($1, $2, $3, $4) RETURNING *', [email, hash, role, name]);
  }
  return db.run('INSERT INTO users (email, password_hash, role, name) VALUES (?, ?, ?, ?)', [email, hash, role, name]);
}
...
app.post('/api/login/:role', loginLimiter, async (req, res) => {
  try {
    ...
    if (role !== 'client') return res.status(404).json({ ok: false, error: 'portal_disabled' });
    if (!email || !password) return res.status(400).json({ ok: false, error: 'missing_fields' });
    if (!await verifyCaptcha(captcha)) return res.status(400).json({ ok: false, error: 'captcha_failed' });
...
app.post('/api/profile', async (req, res) => {
  ...
  await db.run(db.usePostgres
    ...
    : 'UPDATE users SET name = ?, avatar_url = ? WHERE email = ?', [name, avatarUrl, req.session.user.email]);
  res.json({ ok: true, user: { email: req.session.user.email, role: req.session.user.role, name, avatarUrl } });
});
...
app.get('/api/orders', async (req, res) => {
  ...
  const email = req.session.user.email;
  try {
    const rows = await db.query(db.usePostgres ? 'SELECT id, product, status, price, image, download_url FROM orders WHERE buyer_email = $1' : 'SELECT id, product, status, price, image, download_url FROM orders WHERE buyer_email = ?', [email]);
    res.json(rows);
