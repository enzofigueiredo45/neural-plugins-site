    ...
    await db.run(
      db.usePostgres
        ? `INSERT INTO leads (email, name, interest, marketing_opt_in, marketing_consent_at, marketing_consent_version, utm_source, utm_medium, utm_campaign, utm_content, utm_term, landing_page, referrer)
           ...
           ON CONFLICT (email) DO UPDATE SET name = EXCLUDED.name, interest = EXCLUDED.interest, status = 'Novo', marketing_opt_in = leads.marketing_opt_in OR EXCLUDED.marketing_opt_in, marketing_consent_at = CASE WHEN EXCLUDED.marketing_opt_in THEN EXCLUDED.marketing_consent_at ELSE leads.marketing_consent_at END, marketing_consent_version = CASE WHEN EXCLUDED.marketing_opt_in THEN EXCLUDED.marketing_consent_version ELSE leads.marketing_consent_version END, marketing_unsubscribed_at = CASE WHEN EXCLUDED.marketing_opt_in THEN NULL ELSE leads.marketing_unsubscribed_at END, utm_source = EXCLUDED.utm_source, utm_medium = EXCLUDED.utm_medium, utm_campaign = EXCLUDED.utm_campaign, utm_content = EXCLUDED.utm_content, utm_term = EXCLUDED.utm_term, landing_page = EXCLUDED.landing_page, referrer = EXCLUDED.referrer, updated_at = CURRENT_TIMESTAMP`
        : `INSERT INTO leads (email, name, interest, marketing_opt_in, marketing_consent_at, marketing_consent_version, utm_source, utm_medium, utm_campaign, utm_content, utm_term, landing_page, referrer)
           VALUES (?,?,?,?,CASE WHEN ? THEN CURRENT_TIMESTAMP ELSE NULL END,CASE WHEN ? THEN 'lead-form-v2' ELSE NULL END,?,?,?,?,?,?,?)
           ON CONFLICT(email) DO UPDATE SET name = excluded.name, interest = excluded.interest, status = 'Novo', marketing_opt_in = CASE WHEN leads.marketing_opt_in = 1 OR excluded.marketing_opt_in = 1 THEN 1 ELSE 0 END, marketing_consent_at = CASE WHEN excluded.marketing_opt_in = 1 THEN excluded.marketing_consent_at ELSE leads.marketing_consent_at END, marketing_consent_version = CASE WHEN excluded.marketing_opt_in = 1 THEN excluded.marketing_consent_version ELSE leads.marketing_consent_version END, marketing_unsubscribed_at = CASE WHEN excluded.marketing_opt_in = 1 THEN NULL ELSE leads.marketing_unsubscribed_at END, utm_source = excluded.utm_source, utm_medium = excluded.utm_medium, utm_campaign = excluded.utm_campaign, utm_content = excluded.utm_content, utm_term = excluded.utm_term, landing_page = excluded.landing_page, referrer = excluded.referrer, updated_at = CURRENT_TIMESTAMP`,
