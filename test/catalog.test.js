...
test("builds a public catalog without exposing Stripe IDs or access URLs", () => {
  ...
  assert.equal(publicCatalog[0].licenseType, "Licença digital; confirme a modalidade de ativação antes da compra");
  ...
  assert.ok(publicCatalog.every((product) => product.licenseType.includes("confirme")));
