...
function getEmailConfig(env = process.env) {
  return {
    ...
    siteUrl: normalizeSiteUrl(env.SITE_URL) || "https://neural-plugins-site.vercel.app",
  };
}
...
async function sendEmail({ to, subject, html, text, replyTo, key }, env = process.env) {
...
async function sendEmailVerificationEmail({ email, name, token }, env = process.env) {
  const config = getEmailConfig(env);
  const verificationUrl = `${config.siteUrl}/verify-email.html?token=${encodeURIComponent(token)}`;
  return sendEmail({
    to: email,
    subject: "Confirme seu e-mail — Neural X",
    key: `verify-email:${String(email).toLowerCase()}:${token}`,
    text: `Olá, ${name || "Cliente"}. Confirme seu e-mail para proteger sua conta e vincular compras: ${verificationUrl}. O link expira em 24 horas.`,
    html: emailShell({
      preview: "Confirme seu e-mail para proteger sua biblioteca.",
      title: "Confirme seu e-mail.",
      body: `<p style="font-size:16px;line-height:1.65;margin:0 0 12px">Olá, ${escapeHtml(name || "Cliente")}. Esta confirmação prova que o endereço pertence a você antes de exibirmos qualquer compra.</p><p style="font-size:14px;line-height:1.6;margin:0;color:#686875">O link expira em 24 horas. Se você não criou a conta, ignore esta mensagem.</p>`,
...
async function sendPasswordResetEmail({ email, name, token }, env = process.env) {
  const config = getEmailConfig(env);
  const resetUrl = `${config.siteUrl}/reset-password.html?token=${encodeURIComponent(token)}`;
  return sendEmail({
    to: email,
    subject: "Redefina sua senha — Neural X",
    key: `password-reset:${String(email).toLowerCase()}:${token}`,
    text: `Olá, ${name || "Cliente"}. Redefina sua senha em ${resetUrl}. O link expira em 30 minutos. Se você não pediu a troca, ignore esta mensagem.`,
    html: emailShell({
      preview: "Link seguro para redefinir sua senha.",
      title: "Redefina sua senha.",
      body: `<p style="font-size:16px;line-height:1.65;margin:0 0 12px">Olá, ${escapeHtml(name || "Cliente")}. Use o botão abaixo para criar uma nova senha.</p><p style="font-size:14px;line-height:1.6;margin:0;color:#686875">O link expira em 30 minutos e só pode ser usado uma vez. Se você não pediu a troca, ignore esta mensagem.</p>`,
...
async function sendOrderConfirmationEmail({ checkoutId, email, products, amountTotal, currency }, env = process.env) {
  ...
  const productList = products
    ...
    .join("");
  const hasAssistedDelivery = products.some(({ accessMode }) => accessMode !== "automatic");
  const accessMessage = hasAssistedDelivery
    ? "Seu pedido está registrado. A entrega assistida e as instruções de ativação serão concluídas em até 4 horas. Para proteger a compra, o acesso fica na biblioteca da conta com e-mail verificado."
    : "Seu pedido está registrado. Para proteger a compra, o acesso fica na biblioteca da conta com e-mail verificado. A licença digital fica vinculada ao computador usado na ativação.";
  return sendEmail({
    to: email,
    subject: "Pagamento confirmado — Neural X",
    key: `order-confirmation:${checkoutId}`,
    text: `Pagamento confirmado (${total}). ${products.map((item) => `${item.name} x${item.quantity}`).join(", ")}. ${accessMessage}`,
    html: emailShell({
      preview: `Pagamento confirmado no valor de ${total}.`,
      title: "Pagamento confirmado.",
      body: `<p style="font-size:16px;line-height:1.65;margin:0 0 12px">Recebemos seu pagamento de <strong>${escapeHtml(total)}</strong>.</p><ul style="font-size:16px;line-height:1.6;padding-left:20px">${productList}</ul><p style="font-size:16px;line-height:1.65;margin:14px 0 0">${escapeHtml(accessMessage)}</p>`,
...
async function sendRecommendationEmail({ email, name, recommendation }, env = process.env) {
  const config = getEmailConfig(env);
  ...
  return sendEmail({
    ...
    text: `Olá, ${name}. Sua recomendação é ${recommendationName}. ${reason} Veja: ${recommendationUrl}`,
    html: emailShell({
      preview: `Sua recomendação: ${recommendationName}.`,
      title: "Sua recomendação chegou.",
      body: `<p style="font-size:16px;line-height:1.65;margin:0 0 12px">Olá, ${escapeHtml(name)}.</p><p style="font-size:16px;line-height:1.65;margin:0 0 12px">Pelo objetivo que você escolheu, o melhor ponto de partida é <strong>${escapeHtml(recommendationName)}</strong>.</p><p style="font-size:16px;line-height:1.65;margin:0">${escapeHtml(reason)}</p>`,
      ctaLabel: "Ver recomendação",
      ctaUrl: recommendationUrl,
      siteUrl: config.siteUrl,
    }),
  }, env);
}
