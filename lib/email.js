...
async function sendOrderConfirmationEmail({ checkoutId, email, products, amountTotal, currency, accessReady, accessRequestRequired }, env = process.env) {
  const config = getEmailConfig(env);
  ...
  const productList = products
    ...
    .join("");
  const accessMessage = accessReady
    ? "O acesso está disponível na sua biblioteca Neural X."
    : accessRequestRequired
      ? "Entre na sua biblioteca e solicite o acesso; o pedido será conferido antes da aprovação."
      : "O pagamento foi registrado e a liberação será acompanhada na sua biblioteca Neural X.";
  return sendEmail({
    to: email,
