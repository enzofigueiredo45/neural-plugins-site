...
async function sendEmailVerificationEmail(
  { email, name, verificationUrl, requestId = randomUUID() },
  env = process.env,
) {
  const config = getEmailConfig(env);
  const safeUrl = normalizeSiteUrl(verificationUrl);
  if (!safeUrl || !safeUrl.startsWith(`${config.siteUrl}/verify-email.html`))
    throw new Error("Invalid verification URL");
  return sendEmail({
    to: email,
    subject: "Confirme seu e-mail — Neural X",
    key: `verify-email:${requestId}`,
    text: `Olá, ${name || "Cliente"}. Confirme seu e-mail para acessar pedidos e chamados: ${safeUrl} O link vale por 24 horas.`,
    html: emailShell({
      preview: "Confirme seu e-mail para proteger sua biblioteca.",
      title: "Confirme seu e-mail.",
      body: `<p style="font-size:16px;line-height:1.65;margin:0 0 12px">Olá, ${escapeHtml(name || "Cliente")}.</p><p style="font-size:16px;line-height:1.65;margin:0">Use o botão abaixo para confirmar que este endereço é seu. O link vale por 24 horas e só pode ser usado uma vez.</p>`,
...
async function sendOrderConfirmationEmail({ checkoutId, email, products, amountTotal, currency }, env = process.env) {
  ...
  const accessMessage = downloadLinks.length === products.length
    ? "Use os links abaixo e siga as instruções correspondentes ao produto e à edição informados no pedido."
    : "O link de download e as instruções correspondentes ao produto e à edição serão enviados a este e-mail em até 4 horas.";
...
async function sendRecommendationEmail({
  ...
  const greeting = friendlyName ? `Olá, ${friendlyName}.` : "Olá.";
  ...
  const marketingText = marketingConsent && unsubscribeUrl
    ...
    : " Você pediu somente esta recomendação. Este envio não adiciona marketing nem altera inscrições anteriores; para cancelá-las, use o descadastro nos e-mails de conteúdos e ofertas.";
  const marketingHtml = marketingConsent && unsubscribeUrl
    ...
    : `<p style="font-size:13px;line-height:1.6;margin:18px 0 0">Você pediu somente esta recomendação. Este envio não adiciona marketing nem altera inscrições anteriores; para cancelá-las, use o descadastro nos e-mails de conteúdos e ofertas.</p>`;
