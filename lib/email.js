...
async function sendRecommendationEmail({ email, name, recommendation }, env = process.env) {
  ...
  const privacyRequestUrl = `${config.siteUrl}/contact.html?assunto=privacidade`;
  ...
  return sendEmail({
    ...
    text: `Olá, ${name}. Sua recomendação é ${recommendationName}. ${reason} Veja: ${recommendationUrl.href}. Para solicitar o cancelamento de conteúdos e ofertas: ${privacyRequestUrl}.`,
    html: emailShell({
      preview: `Sua recomendação: ${recommendationName}.`,
      title: "Sua recomendação chegou.",
      body: `<p style="font-size:16px;line-height:1.65;margin:0 0 12px">Olá, ${escapeHtml(name)}.</p><p style="font-size:16px;line-height:1.65;margin:0 0 12px">Pelo objetivo que você escolheu, um ponto de partida é <strong>${escapeHtml(recommendationName)}</strong>.</p><p style="font-size:16px;line-height:1.65;margin:0">${escapeHtml(reason)}</p><p style="font-size:13px;line-height:1.6;margin:18px 0 0">Você pediu esta recomendação no site. Para deixar de receber conteúdos e ofertas, <a href="${escapeHtml(privacyRequestUrl)}">solicite o cancelamento ao suporte</a>. Isso não altera seus pedidos.</p>`,
