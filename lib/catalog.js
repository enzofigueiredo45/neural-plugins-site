...
function getProductCatalog(env = process.env) {
  const usesLiveStripe = /^(?:sk|rk)_live_/.test(
    String(env.STRIPE_SECRET_KEY || ""),
  );
  return Object.fromEntries(
    Object.values(PRODUCT_DEFINITIONS).map((definition) => {
      ...
      return [definition.id, {
        ...
        price: String(
          usesLiveStripe && definition.livePrice
            ? definition.livePrice
            : env[definition.priceEnv] || "",
        ).trim(),
        accessUrl,
        ...
        saleReady:
          definition.offerDetailsConfirmed === true &&
          String(env.COMMERCIAL_SALES_ENABLED || "").toLowerCase() === "true",
      }];
    }),
  );
}
