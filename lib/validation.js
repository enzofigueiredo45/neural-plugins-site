...
function isMercadoPagoAccessToken(value, allowTest = false) {
  const pattern = allowTest
    ? /^(?:APP_USR|TEST)-[A-Za-z0-9_-]{16,}$/
    : /^APP_USR-[A-Za-z0-9_-]{16,}$/;
  return pattern.test(String(value || ""));
}

function isMercadoPagoWebhookSecret(value) {
  return /^[!-~]{16,256}$/.test(String(value || ""));
}
...
function validateRuntimeConfig(env, isProduction) {
  ...
  if (!env.MERCADO_PAGO_ACCESS_TOKEN)
    warnings.push("MERCADO_PAGO_ACCESS_TOKEN");
  else if (
    ...
    errors.push("MERCADO_PAGO_ACCESS_TOKEN");
  if (!env.MERCADO_PAGO_WEBHOOK_SECRET)
    warnings.push("MERCADO_PAGO_WEBHOOK_SECRET");
  else if (!isMercadoPagoWebhookSecret(env.MERCADO_PAGO_WEBHOOK_SECRET))
    errors.push("MERCADO_PAGO_WEBHOOK_SECRET");
