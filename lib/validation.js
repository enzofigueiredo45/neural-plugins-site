...
function validateRuntimeConfig(env, isProduction) {
  ...
  if (String(env.CRON_SECRET || "").length < 32) warnings.push("CRON_SECRET");
  if (String(env.COMMERCIAL_SALES_ENABLED || "").toLowerCase() !== "true")
    warnings.push("COMMERCIAL_SALES_ENABLED");
