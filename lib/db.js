...
const fs = require("fs");
const { migrateSqliteLeadConsent } = require("./lead-storage");
...
async function initPostgres() {
  ...
  // Missing evidence is not permission. Do not infer opt-in from a legacy row.
  // Existing explicit choices and historical evidence are kept untouched.
  await pgPool.query(
...
async function initSqlite() {
  ...
  await run(
    "UPDATE leads SET marketing_opt_in = 0 WHERE marketing_opt_in IS NULL",
  );
  await migrateSqliteLeadConsent({ query, getOne, run });
  await run(
